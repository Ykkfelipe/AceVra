...
export function spawnHostProcess(
  ...
  const child = electronUtilityProcess.fork(hostModulePath, [], {
    ...
    env: {
      ...
      // code-signing identity is a nested Electron helper (NOT dev.acevra.app). Publish THIS (main
      // Electron) process's pid — which IS dev.acevra.app — so helperLauncher passes it as
