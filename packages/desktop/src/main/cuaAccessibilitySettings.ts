...
async function verifyHelperPermissionIdentityUnchanged(
  ...
  if (!cuaHelperBundleFingerprintUnchanged(identity.appPath, fingerprint)) {
    throw new Error(`AceVra Computer Use changed while its ${phase} signature was being verified`);
  }
  ...
  if (!sameHelperPermissionIdentity(identity, currentIdentity)) {
    throw new Error(`AceVra Computer Use permission identity changed during ${phase} verification`);
  }
  if (!cuaHelperBundleFingerprintUnchanged(identity.appPath, fingerprint)) {
    throw new Error(
      `AceVra Computer Use changed while its ${phase} permission identity was being resolved`,
    );
  }
  return fingerprint;
}
...
export async function openCuaPermissionOnboarding(
  ...
  if (platform !== "darwin") {
    return {
      success: false,
      error: "AceVra Computer Use permissions are only available on macOS.",
    };
  }
  ...
  try {
    ...
    return {
      success: false,
      error: `AceVra Computer Use is unavailable (install/verification failed): ${messageOf(error)}`,
    };
  }
  ...
  try {
    ...
    return {
      success: false,
      returnedFromSettings: false,
      error: `AceVra Computer Use permission identity verification failed: ${messageOf(error)}`,
    };
  }
...
export async function prepareCuaHelperPermissionDrag(
  ...
  try {
    ...
    if (!cuaHelperBundleFingerprintUnchanged(helperAppPath, verifiedFingerprint)) {
      throw new Error("AceVra Computer Use changed while its drag signature was being verified");
    }
    const identity = await (
      options.resolveHelperIdentity ?? resolveHelperPermissionSubjectIdentity
    )(helperAppPath);
    if (!cuaHelperBundleFingerprintUnchanged(helperAppPath, verifiedFingerprint)) {
      throw new Error("AceVra Computer Use changed while its drag identity was being resolved");
    }
    return {
      success: true,
      helperAppPath,
