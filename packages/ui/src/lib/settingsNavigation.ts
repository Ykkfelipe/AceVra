...
const HIDDEN_SETTINGS_SECTIONS = new Set<SettingsSectionId>([
  ...
  "plugins",
  // 旧顶层「账号与导入」分区已并入模型设置 split panel（account 导航项）；
  // resolveSettingsSection 会把该 id 迁到 modelProvider，这里兜底防再入选项列表。
  "accounts",
...
export function resolveSettingsSection(
  ...
  // 偏好/意图迁移：账号与导入已并入模型设置 split panel 的 account 导航项，
  // 旧 "accounts" 顶层分区 id 统一落到 modelProvider（plugins→plugin 同款处理）。
  if (section === "accounts") return "modelProvider";
